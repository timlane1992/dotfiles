...
function! s:TreeExplorer(split, start) " <<<
  ...
  nnoremap <buffer> <cr> :call <SID>Activate("win")<cr>:<bs>
  nnoremap <buffer> o    :call <SID>Activate("win")<cr>:<bs>
  nnoremap <buffer> O    :call <SID>Activate("cur")<cr>:<bs>
	nnoremap <buffer> t    :call <SID>Activate("tab")<cr>:<bs>
	nnoremap <buffer> X    :call <SID>RecursiveExpand()<cr>:<bs>
	nnoremap <buffer> E    :call <SID>OpenExplorer()<cr>:<bs>
  nnoremap <buffer> C    :call <SID>ChangeTop()<cr>:<bs>
  nnoremap <buffer> H    :call <SID>InitWithDir($HOME)<cr>:<bs>
	nnoremap <buffer> u    :call <SID>ChdirUp()<cr>:<bs>
	nnoremap <buffer> p    :call <SID>MoveParent()<cr>:<bs>
	nnoremap <buffer> r    :call <SID>RefreshDir()<cr>:<bs>
  nnoremap <buffer> R    :call <SID>InitWithDir("")<cr>:<bs>
	nnoremap <buffer> S    :call <SID>StartShell()<cr>:<bs>
	nnoremap <buffer> D    :call <SID>ToggleDirSort()<cr>:<bs>
	nnoremap <buffer> a    :call <SID>ToggleHiddenFiles()<cr>:<bs>
  nnoremap <buffer> ?    :call <SID>ToggleHelp()<cr>:<bs>
	nnoremap <buffer> <2-leftmouse> :call <SID>Activate("win")<cr>:<bs>
...
function! s:RecursiveExpand() " <<<
	...
	let curfile = GetAbsPath2(line("."), 0)
...
function! GetAbsPath2(ln,ignore_current) " <<<
	let lnum = a:ln
	let l = getline(lnum)
